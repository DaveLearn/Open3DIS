...
### DEG external segmenter wrapper

This repo is used as an external segmenter in DEG. The wrapper script is
`dependencies/Open3DIS/segment.py` and mirrors the SAI3D mesh/workspace
pipeline to generate ScanNet-like inputs before running Open3DIS.

Two baselines are supported:

- **2D-only (fair baseline)**: no 3D proposals and no dc_features.
...
- `--use-superpoints` requires `dc_features` (generated by ISBNet).
